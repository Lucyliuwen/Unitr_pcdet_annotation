...
class DSVTInputLayer(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.sparse_shape_list = [self.sparse_shape] #sparse_shape: [32, 88, 1](image); [360, 360, 1](lidar)
        ...
        for i in range(len(self.set_info)): #  set_info: [[90, 4]] # sparse_shape: [32, 88, 1]
            input_dim = 3 if self.sparse_shape_list[i][-1] > 1 else 2  # image: 3
            ...
            for j in range(self.set_info[i][1]):
                ...
                for s in range(self.num_shifts[i]):
                    block_posembed_layers.append(PositionEmbeddingLearned(input_dim, self.d_model[i])) # d_model: [128]
    ...
    def get_set(self, voxel_info, stage_id):
        ...
        logging.info(f"Set shape: {set_voxel_inds_shift0.shape}")

        # compute key masks, voxel duplication must happen continuously
        prefix_set_voxel_inds_s0 = torch.roll(set_voxel_inds_shift0.clone(), shifts=1, dims=-1) #shifts: -1向左，1向右
        # 将得到的结果向后循环移动一个位置，这样就可以得到与原先的set_voxel_inds_shift0向量形状相同，【每个元素都是位于其前一个元素的位置】 相当于得到前缀元素
        logging.info(f"prefix_set_voxel_inds_s0: {prefix_set_voxel_inds_s0.shape}")
        
        prefix_set_voxel_inds_s0[ :, :, 0] = -1 # 将最后一个维度的第一个元素设置为-1。
        # 为了与set_voxel_inds_shift0的第一个元素不匹配，从而让mask的第一个元素为False
        logging.info(f"prefix_set_voxel_inds_s0: {prefix_set_voxel_inds_s0.shape}, {prefix_set_voxel_inds_s0}")

        set_voxel_mask_s0 = (set_voxel_inds_shift0 == prefix_set_voxel_inds_s0) # 就是这个set_voxel_mask_s0的第一个元素为0
        logging.info(f"set_voxel_mask_s0: {set_voxel_mask_s0.shape}, {set_voxel_mask_s0}")
        # 用这个掩码表示，set_voxel_inds_shift0 中每个元素是否与前一个元素想等，true=相等
        '''
            这个prefix_set_voxel_inds_s0实际上就是为了得到这个set_voxel_mask_s0的
            用前一个元素与当前元素是否相等来当作掩码 相等为True 
            在mask机制中 mask=True的部分在后期计算时是不需要的 为了便于计算才使用mask 

            问题: 这里通过前一个元素与后一个元素是否相等来设置掩码 这样做的目的是什么呢
            我能想到的一个好处就是 同一个元素一次只参与到一个元素的自注意力计算当中来提高计算效率和降低计算复杂度
        '''
        ...
        logging.info(f"Set shape: {set_voxel_inds_shift1.shape}")
    ...
    def get_set_single_shift(self, batch_win_inds, stage_id, shift_id=None, coors_in_win=None):
        ...
        voxel_num_set = self.set_info[stage_id][0] # 每个set里面包含90个voxel
        # max number of voxels in a window # 1个window里面最多包含的voxel window_shape: [[30, 30, 1]] = 30 * 30 * 1 = 90 计算出来的voxel 最大数量
        ...
        logging.info(f"batch_win_inds: {batch_win_inds.shape}, {batch_win_inds}")
        # get unique set indexs
        contiguous_win_inds = torch.unique(batch_win_inds, return_inverse=True)[1] # 返回张量中的唯一元素，return_inverse=True 表示返回唯一元素张量的索引，
        # torch.unique(batch_win_inds, return_inverse=True) 返回两个张量，一个是包含所有唯一元素的张量，另一个是新的索引的张量
        # [1]则是用来获取这个索引张量
        logging.info(f'contiguous_win_inds: {contiguous_win_inds.shape}, {contiguous_win_inds}')

        voxelnum_per_win = torch.bincount(contiguous_win_inds)
        win_num = voxelnum_per_win.shape[0]
        setnum_per_win_float = voxelnum_per_win / voxel_num_set # 得到set的数量
        ...
        global_voxel_inds_sortx = contiguous_win_inds * max_voxel + \
                ...
                coors_in_win[:,0]
        _, order2 = torch.sort(global_voxel_inds_sortx) # 将全局索引排序

        inner_voxel_inds_sortx = -torch.ones_like(inner_voxel_inds) # 用于存储体素在排序后的窗口内的索引
        ...
        all_set_voxel_inds = torch.stack((set_voxel_inds_sorty, set_voxel_inds_sortx), dim=0) # 这是将按照xy划分排序 加到一起
        return all_set_voxel_inds

    @torch.no_grad()
    def window_partition(self, voxel_info, stage_id):
        logging.info('-----------window partition-----')
        for i in range(2):      #sparse_shape_list = [[32, 88, 1]] / [[360, 360, 1]]
            logging.info(f'window_partition(): ----{i}')
            ...
            voxel_info[f'batch_win_inds_stage{stage_id}_shift{i}'] = batch_win_inds # 全局坐标，复制给batch_win_inds_stage_shift 
            voxel_info[f'coors_in_win_stage{stage_id}_shift{i}'] = coors_in_win

            '''
                问题： 处理的是所有的体素，但是为什么写的是batch呢
            '''
