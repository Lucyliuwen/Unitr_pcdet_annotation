        ...
        Returns:
            tuple: Contains merged results and its spatial shape.
                ...
                - out_size (tuple[int]): Spatial shape of x, arrange as
                    (out_h, out_w).
        """
        logging.info("==================== Image to Patch Embedding =======================")
        logging.info(f"x shape: {x.shape}")         # 18, 3, 256, 704
        if self.adap_padding:
            x = self.adap_padding(x)

        logging.info(f"x shape After adaptive padding : {x.shape}") # 18, 3, 256, 704

        x = self.projection(x)

        logging.info(f"x shape After Conv2d: {x.shape}") #  [18, 128, 32, 88]
        
        out_size = (x.shape[2], x.shape[3])
        x = x.flatten(2).transpose(1, 2)
        if self.norm is not None:
            x = self.norm(x)
        logging.info(f"x shape: {x.shape}") # [18, 2816, 128]
        return x, out_size
